...
const DocUploader = () => {
  ...
  const submitHandler = async(e)=>{
    e.preventDefault();
    try{
    const res = await axios.post(`${url}/api/driver/docUpload`,{vehicleCategory,model,reg,index})
        ...
        {index == 3 ?
          <div className="h-96 scrollbar-hide overflow-y-auto mt-3 p-7 flex-col  flex justify-center ">
            <form onSubmit={submitHandler}>
              ...
              <select className="w-full py-4 border  border-gray-300  rounded-lg  focus:border-gray-200  mb-5" 
              ...
              <label  className="items-start" htmlFor="Model">Vehicle Model</label>
              <input value={model} onChange={e => setModel(e.target.value)} type="text" className="w-full pl-3 focus:shadow-lg shadow-black focus:border-gray-400  py-4 border border-gray-300 rounded-lg mb-5" placeholder="model name" id='model' />
              <label htmlFor="reg.no">Registration Number</label>
              <input value={reg} onChange={e => setReg(e.target.value)} type="text" className="w-full pl-3 py-4 border border-gray-300 rounded-lg mb-5" placeholder="vehicle registration number" id='reg.no' />
              <button
              type="submit"
