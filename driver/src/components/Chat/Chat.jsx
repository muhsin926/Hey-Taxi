...
const Chat = () => {
    ...
    const submitHandler = async e => {
        ...
        const { data } = await axios.post(`${import.meta.env.VITE_SERVER_DOMAIN}/api/driver/chat`, { chatInput, toUserId }, {
    ...
    const getChater = async()=> {
        const token = localStorage.getItem('token')
        const { data } = await axios.get(`${import.meta.env.VITE_SERVER_DOMAIN}/api/driver/getChater`, {
            headers: { Authorization: `Bearer ${token}` }
        })
    ...
    const getMsgs = async() => {
        const token = localStorage.getItem('token')
        const { data } = await axios.get(`${import.meta.env.VITE_SERVER_DOMAIN}/api/driver/chat?passengerId=${toUserId}`, {
            headers: { Authorization: `Bearer ${token}` }
        })
        setMasseges(data)
    }
                ...
                <div className='h-[22rem] overflow-y-auto'>
                    {messages?.map((msg) => (
                        ...
                        <div className=' my-5 w-full flex justify-end'>
                            <h1 className=' bg-gradient-to-br from-cyan-400 to-cyan-800 p-3 mr-2 rounded-t-xl rounded-bl-xl '>{msg.message}</h1>
                        </div>
                        :  <div className=' my-5 w-full flex justify-start'>
                        <h1 className='bg-gradient-to-bl from-orange-400 to-orange-700 p-3 ml-2 rounded-t-xl rounded-br-lg'>{msg.message}</h1>
                    </div>
